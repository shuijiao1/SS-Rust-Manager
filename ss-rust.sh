...
menu(){ need_root; ensure_deps; while true; do clear || true; say "ss-rust 管理脚本 v$VERSION"; say "状态：$(status_line)"; say "1) 安装/更新 ss-rust"; say "2) 修改配置"; say "3) 查看配置"; say "4) 启动"; say "5) 停止"; say "6) 重启"; say "7) 状态"; say "8) 更新脚本"; say "9) 卸载"; say "0) 退出"; read -rp "请选择: " c; case "$c" in 1) install_ss;; 2) modify_config;; 3) view_config;; 4) service_ctl start;; 5) service_ctl stop;; 6) service_ctl restart;; 7) service_ctl status;; 8) check_update;; 9) uninstall_ss;; 0) exit 0;; *) err "无效选项";; esac; read -rp "按回车继续..." _; done; }
